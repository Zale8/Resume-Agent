...
存放 Resume Agent 的程序代码：简历库路径发现、只读解析、通用排版积木与骨架。
DOCX 成品**不是由本目录直接渲染的**——v0.4 起每份简历由 Agent 用 `layout_kit`
的排版原语 + `skeletons` 的骨架，**按岗位动态构建**（见 [AGENT.md §十四](../AGENT.md)）；
仅当需要骨架未覆盖的全新版式时，才编写一次性 python-docx 脚本，脚本用完即删、不在本目录。
...
└── resume_generator/
    ...
    └── skeletons.py      # 三种可复用骨架：build_document(blocks, skeleton_id, palette_id)
```
...
| `skeletons.py` | 三种骨架（双栏侧边栏 / 横幅工牌卡 / 单栏极简） | 先定骨架再定色板；核心板块顺序固定；禁止「旧骨架换色冒充新设计」 |
...
| 版本 | 内容 |
|---|---|
| v0.4 | ✅ 已沉淀动态排版的**通用**辅助函数与三种骨架（`layout_kit.py` / `skeletons.py`，不含任何版式成品）（当前版本） |
