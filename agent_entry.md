...
> **Design Decision ≠ DesignSpec**（本节关键概念，2026-09-19 起）：
> - **Design Decision（本份设计决策）**：Agent 在生成前对「这一份简历怎么设计」做出的
>   判断，是**生成过程中的中间决策**——对话内明确即可，**不新建任何决策文件**；
...
2. **确定 Design Decision**（本份设计决策）：
   - **输入**：JD 分析 + resume.md 内容 + `99_配置/style\_preferences.md`
     ...
     + 单页/内容预算；
   - **输出**（对话内明确即可）：范式（P1/P2/P3）、palette、**photo decision**
   ...
   - 骨架/色板只是**范式级**选择，**不等于完整的本份设计**；本份照片坐标、
     尺寸等具体参数只进 Design Decision，**禁止写入 Preset / 产品代码 / 全局配置**。
...
7. **单页实测**：Word/WPS COM `doc.ComputeStatistics(2)` 读真实页数：
   - \= 1 → 通过；
   - <br />
     > 1 → 按 1.4 优先级删内容/收间距，**更新 Design Decision 后重新装配生成**
     （正文不低于 9pt），反复到 = 1；
...
9. **写 generation\_notes.md**（生成后 **Record**）：公司/岗位、JD、实际采用的设计与
   ...
   下一次生成的设计输入是当次重新确定的 Design Decision，不机器读取上一份 notes。
...
当前已封存一份 **Golden Sample**：
`简历库/09_岗位定制简历/某涂料企业/AI技术与智能化工程师/2026-09-18/golden_sample.md`
（同目录含只读封存 DOCX、视觉规范、人工修改 Change Log 与 8 条验收规则）。

定位与边界：

- ✅ **只读视觉回归基准**：生成同类视觉范式的简历后，用它校验视觉结果
  （页数、末行阈值、分割线、照片处理等验收规则见其 §三）。
- ✅ **视觉验收规则的来源之一**：其「产品化分级」记录了已确认的产品级规则
...
| 「换个风格/太丑」   | 更新 Design Decision 重新装配生成；事实层不动                        |
